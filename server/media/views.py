...
class RevenueList(generics.ListCreateAPIView):
  ...
  def get(self, request, format=None):
    dataTime = datetime.now().date()
    dayli = Revenue.objects.get(added_at=dataTime)
    sum_1 = 0
    sum_2 = 0
    sum_3 = 0
    sum_4 = 0
    sum_5 = 0
    week1 = Revenue.objects.filter()[1:5]
    week2 = Revenue.objects.filter()[5:10]
    week3 = Revenue.objects.filter()[10:15]
    week4 = Revenue.objects.filter()[15:20]
    week5 = Revenue.objects.filter()[20:25]
    for i in week1:
      sum_1 += i.cash_income
    for i in week2:
      sum_2 += i.cash_income
    ...
    for i in week5:
      sum_5 += i.cash_income
    serializer = RevenueSerializer(dayli, many=True)
    return Response(sum_1, sum_2, sum_3, sum_4, sum_5, data=serializer.data, status=status.HTTP_200_OK  )
